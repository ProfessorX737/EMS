...
from wtforms import TextField, TextAreaField, validators, StringField, DateField, SubmitField, SelectField
...
class CreateEventForm(Form):
    ...
    startDateTime = DateField('Start Date Time', format='%d-%m-%Y %H:%M', validators=[validators.DataRequired("Please enter the event start date and time.")])
    endDateTime = DateField('End Date Time', format='%d-%m-%Y %H:%M', validators=[validators.DataRequired("Please enter the event end date and time.")])
    deregEnd = DateField('Deregister Period End', format='%d-%m-%Y %H:%M', validators=[validators.DataRequired("Please enter the end of deregistration period.")])
